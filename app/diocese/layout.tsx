...
const dioceseNav = [
  { name: 'Dashboard', href: '/diocese', icon: HiOutlineHome },
  { name: 'Circuits', href: '/diocese/circuits', icon: HiOutlineOfficeBuilding },
  { name: 'Superintendent Ministers', href: '/diocese/superintendents', icon: HiOutlineUserGroup },
  { name: 'Circuit Statistics', href: '/diocese/statistics', icon: HiOutlineChartBar },
  { name: 'Financial Reports', href: '/diocese/financial-reports', icon: HiOutlineDocumentReport },
  { name: 'Circuit Performance', href: '/diocese/performance', icon: HiOutlineTrendingUp },
];
...
export default function DioceseLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (user && user.roleId !== SYSTEM_ROLE_IDS.DIOCESE_ADMIN && user.roleId !== SYSTEM_ROLE_IDS.HEAD_PASTOR) {
      router.push(user.roleId === SYSTEM_ROLE_IDS.CIRCUIT_ADMIN ? '/circuit' : '/dashboard');
    }
  ...
  if (user.roleId !== SYSTEM_ROLE_IDS.DIOCESE_ADMIN && user.roleId !== SYSTEM_ROLE_IDS.HEAD_PASTOR) {
    return null;
  }
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      >
        <div className="h-16 flex items-center justify-between px-4 border-b border-gray-200">
          {sidebarOpen && (
            <div className="flex items-center gap-3">
              ...
              <span className="font-semibold text-gray-800">Diocese</span>
            </div>
          )}
          {!sidebarOpen && (
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-1">
          {dioceseNav.map((item) => {
            const isActive = pathname === item.href || (item.href !== '/diocese' && pathname?.startsWith(item.href));
          ...
          <div className="lg:hidden fixed inset-0 z-40 bg-black/50" onClick={() => setMobileMenuOpen(false)}>
            <div className="bg-white w-64 h-full shadow-xl" onClick={(e) => e.stopPropagation()}>
              <div className="p-4 border-b border-gray-200 flex justify-between items-center">
                <span className="font-semibold">Diocese Menu</span>
